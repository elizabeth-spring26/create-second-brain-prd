...
TOOLS = [
    {
        ...
        "description": "Schedule a reminder to be sent to Elizabeth at a specific future time.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "remind_at": {
                    "type": "string",
                    "description": "Absolute datetime in ISO 8601 format (America/New_York local time), e.g. '2026-07-30T09:00:00'. Compute this from the current time given in context plus any relative phrase she used.",
                },
            },
...
def _run_tool(name, tool_input):
    ...
    if name == "create_reminder":
        ...
        return f"Reminder set: \"{r['text']}\" at {r['remind_at']} (id {r['id']})"

    if name == "list_reminders":
        pending = [r for r in reminders if r["status"] == "pending"]
        if not pending:
            return "No pending reminders."
        return "\n".join(f"- [{r['id']}] {r['text']} at {r['remind_at']}" for r in pending)
...
schedule, list, or cancel reminders. If she asks to be reminded of something but the timing is ambiguous,
